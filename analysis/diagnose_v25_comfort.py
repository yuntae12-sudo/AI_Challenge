...
#
# IMPORTANT:
# Keep the same batch_size=64 ordering used by the original
# secondary evaluation so scenario_index matches the CSV.
# ============================================================
...
for batch in data_generator:
    ...
    # Original evaluation uses the first dimension as batch.
    batch_size_now = int(
        batch.object_metadata.is_sdc.shape[0]
    )

    # Reproduce the official evaluator RNG exactly:
    #
    # rng_key, scenario_key = split(rng_key)
    # scenario_keys = split(scenario_key, batch_size)
    rng, batch_key = jax.random.split(
        rng
    )

    scenario_keys = jax.random.split(
        batch_key,
        batch_size_now,
    )
    ...
    for local_idx in range(
        ...
        scenario = jax.tree.map(
            lambda x: x[local_idx],
            batch,
        )
        ...
        # This is exactly the RNG received by one
        # run_scenario_jit invocation under vmap.
        scenario_rng = scenario_keys[
            local_idx
        ]

        scenario_rng, reset_key = (
            jax.random.split(
                scenario_rng
            )
        )

        # Match run_scenario_jit:
        # if scenario has an inner batch dimension,
        # reset gets one key per inner element.
        if scenario.shape != ():
        ...
        while not bool(
            ...
            # Match run_scenario_jit exactly.
            scenario_rng, step_key = (
                jax.random.split(
                    scenario_rng
                )
            )
...
# ============================================================
# SCENARIO ALIGNMENT SANITY CHECK
# ============================================================

trace_episode_comfort = (
    trace
    .groupby("scenario_index")[
        "comfort_step"
    ]
    .mean()
)
...
alignment = pd.DataFrame(
    {
        ...
        "csv_comfort":
            expected_comfort.loc[
                trace_episode_comfort.index
            ],
    }
)

alignment["abs_error"] = abs(
    alignment["trace_comfort"]
    - alignment["csv_comfort"]
)
...
print(
    "Mean abs error     :",
    f"{alignment['abs_error'].mean():.8f}",
)

print(
    "Max abs error      :",
    f"{alignment['abs_error'].max():.8f}",
)
...
alignment.to_csv(
    "analysis/"
    "v25_diagnostic_alignment_check.csv"
)
